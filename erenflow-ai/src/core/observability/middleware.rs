...
impl ObservabilityMiddleware {
    ...
    fn extract_token_usage(state: &State) -> Option<TokenUsage> {
        state.get(TOKEN_USAGE_STATE_KEY).and_then(|v| {
    ...
    fn state_to_snapshot(state: &State, max_size: usize) -> Option<String> {
