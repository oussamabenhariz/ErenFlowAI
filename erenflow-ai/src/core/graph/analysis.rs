...
impl GraphAnalyzer {
    ...
    /// }
    /// ```
    pub fn analyze(graph: &Graph) -> Result<BranchAnalysis> {
    ...
    fn find_sequential_head(graph: &Graph) -> Result<Vec<NodeId>> {
    ...
    fn find_branch_points(graph: &Graph) -> Vec<NodeId> {
        let mut branch_points = vec![];

        for (node_id, _) in graph.nodes() {
            let outgoing = graph.get_next_nodes(node_id);
            if outgoing.len() > 1 {
                branch_points.push(node_id.clone());
            }
        }
    ...
    fn trace_branches(graph: &Graph, branch_point: &str) -> Result<Vec<Vec<NodeId>>> {
    ...
    fn find_sequential_tail(graph: &Graph, branches: &[Vec<NodeId>]) -> Result<Vec<NodeId>> {
        let mut sequential = vec![];
    ...
    fn find_join_point(graph: &Graph, branches: &[Vec<NodeId>]) -> Result<Option<NodeId>> {
        ...
        for (node_id, _) in graph.nodes() {
            ...
            for branch in branches {
                if !branch.contains(node_id) && !Self::is_reachable_from(graph, branch.last(), node_id)? {
    ...
    fn is_reachable_from(graph: &Graph, start: Option<&NodeId>, target: &str) -> Result<bool> {
