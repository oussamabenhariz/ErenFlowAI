...
impl Graph {
    /// Create a new empty graph with acyclic enforcement (DAG mode)
    ///
    /// # Example
    ///
    /// ```ignore
    /// use erenflow_ai::core::graph::Graph;
    ///
    /// let mut graph = Graph::new();
    /// // graph will validate that no cycles exist on graph.validate()
    /// ```
    pub fn new() -> Self {
        Graph {
            nodes: HashMap::new(),
    ...
    ///
    /// # Errors
    /// Returns `ErenFlowError` if validation fails with detailed reason
    ///
    /// # Performance
    ...
    /// * `Err(ErenFlowError::CycleDetected)` - If graph has cycles
    ///
    /// # Performance
    /// * Time: O(V + E)
    /// * Space: O(V)
...
///
/// ```ignore
/// use erenflow_ai::core::graph::GraphBuilder;
/// use erenflow_ai::core::node::Node;
///
...
///
/// ```ignore
/// use erenflow_ai::core::graph::{GraphBuilder, RoutingCondition, Condition, ComparisonOp};
/// use erenflow_ai::core::node::Node;
///
...
impl GraphBuilder {
    ...
    pub fn add_edge_with_condition(
        ...
        match condition {
            routing::RoutingCondition::Function(f) => {
                let edge = crate::core::node::Edge::new(from_str, to_str, Some(f));
                self.graph.add_edge(edge);
            }
            routing::RoutingCondition::DSL(cond) => {
                // For DSL-based conditions, create an edge and set the routing condition
                let mut edge = crate::core::node::Edge::new(from_str, to_str, None);
    ...
    pub fn add_step(
        ...
        let edge = crate::core::node::Edge::new(from_str, to_str, None);
        self.graph.add_edge(edge);
        self
    }
    ...
    pub fn build_and_validate(self) -> Result<Graph> {
