...
fn compute_layout(graph: &Graph) -> (Vec<Node>, f32, f32) {
    ...
    for (node_id, _) in nodes.iter() {
        levels.insert(node_id.clone(), 0usize);
    }
    ...
    for (node_id, _) in nodes.iter() {
        ...
        level_nodes[level].push((node_id.clone(), radius));
    }
...
pub fn visualize_graph(graph: &Graph, config: VisualizationConfig) -> Result<()> {
...
pub fn visualize_graph_with_execution(
    graph: &Graph,
...
fn visualize_graph_inner(
    ...
    let path_set: std::collections::HashSet<String> = exec_path
        ...
        .unwrap_or_default();
    let path_vec = exec_path.as_ref().map(|p| p.as_slice()).unwrap_or(&[]);
    ...
    svg::save(&config.output_path, &document)
        .map_err(|_| crate::core::error::ErenFlowError::GraphError("Failed to save SVG".into()))?;
