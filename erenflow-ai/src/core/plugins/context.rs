...
pub struct PluginContext {
    ...
    data: Arc<std::sync::Mutex<std::collections::HashMap<String, Arc<dyn std::any::Any + Send + Sync>>>>,
}
...
impl PluginContext {
    ...
    pub fn new() -> Self {
        ...
        Self {
            state: Arc::new(State::new(plain_state)),
    ...
    pub fn set_data<T: 'static + Send + Sync>(&self, key: &str, value: T) -> Result<()> {
        let mut data = self
            .data
            .lock()
            .map_err(|_| ErenFlowError::RuntimeError("Plugin context lock poisoned".to_string()))?;
    ...
    pub fn get_data<T: 'static + Send + Sync>(&self, key: &str) -> Result<Option<Arc<T>>> {
        let data = self
            ...
            .lock()
            .map_err(|_| ErenFlowError::RuntimeError("Plugin context lock poisoned".to_string()))?;
        Ok(data.get(key).and_then(|arc: &Arc<dyn std::any::Any + Send + Sync>| {
            arc.clone()
                .downcast::<T>()
                .ok()
        }))
