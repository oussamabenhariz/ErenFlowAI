/// Plugin system for ErenFlowAI extensibility
///
/// This module provides a plugin architecture for extending ErenFlowAI with
...
pub use registry::{PluginRegistry, PluginId};
pub use context::PluginContext;

/// The main plugin trait that all plugins must implement
///
/// Plugins can hook into various lifecycle events and extend ErenFlowAI's
...
/// use erenflow_ai::plugins::Plugin;
///
/// struct MyPlugin;
///
/// #[async_trait]
...
pub trait Plugin: Send + Sync {
    ...
    async fn on_handler_error(
        ...
        _context: &PluginContext,
        _handler_name: &str,
        _error: &ErenFlowError,
    ) -> Result<()> {
        Ok(())
    }

    /// Called before node execution
    async fn on_node_execute(
        &self,
        _context: &PluginContext,
        _node_id: &str,
    ) -> Result<()> {
