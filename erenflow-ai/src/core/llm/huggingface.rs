    ...
    fn build_payload(&self, messages: Vec<Message>) -> serde_json::Value {
        ...
        match &self.mode {
            HuggingFaceMode::LocalTgi { .. } => {
                ...
                if let Some(obj) = payload.get_mut("parameters").and_then(|p| p.as_object_mut())
                {
...
impl LLMClient for HuggingFaceClient {
    async fn chat(&self, messages: Vec<Message>) -> crate::core::error::Result<Message> {
        ...
        let data: serde_json::Value = response.json().await.map_err(|e| {
            ErenFlowError::LLMError(format!("Failed to parse HuggingFace response: {}", e))
        })?;
    ...
    async fn chat_with_usage(
        ...
        // TODO: Extract usage from TGI responses if available
        Ok((msg, None))
    }
    ...
    async fn chat_stream(
        ...
        // TODO: Implement HuggingFace streaming with Server-Sent Events (SSE)
        // This requires streaming responses from TGI endpoint
        Err(ErenFlowError::LLMError(
